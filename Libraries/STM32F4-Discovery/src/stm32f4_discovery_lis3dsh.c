...
void lis3dsh_int1_enable() {
	...
	tmp |= (1 << _LIS3DSH_DR_EN) | (1 << _LIS3DSH_IEA) | (1 << _LIS3DSH_INT1_EN);
...
lis3dsh_register lis3dsh_read_register ( lis3dsh_register address ) {
	lis3dsh_register buffer;

	address |= (uint8_t) READWRITE_CMD;
	LIS3DSH_CS_LOW();
	_lis3dsh_sendbyte ( address );
...
void lis3dsh_write_register ( lis3dsh_register address, lis3dsh_register value ) {
	LIS3DSH_CS_LOW();
	_lis3dsh_sendbyte ( address );
...
void lis3dsh_write_block ( lis3dsh_register* buf, lis3dsh_register address, lis3dsh_register size ) {
	while ( size > 0x00 ) {
		lis3dsh_write_register ( address, *buf );
		size--;
		buf++;
		address++;
	}
...
void lis3dsh_read_block ( uint8_t* buf, uint8_t address, uint16_t size ) {
	while ( size > 0x00 ) {
		*buf = lis3dsh_read_register ( address );
		size--;
		buf++;
		address++;
	}
