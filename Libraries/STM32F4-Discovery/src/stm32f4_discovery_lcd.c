...
void LCD_CtrlLinesConfig ( void ) {
	...
	/* Enable GPIOB, GPIOD, GPIOE and AFIO clocks */
...
#ifndef OPEN407VD
	GPIO_PinAFConfig ( GPIOE, GPIO_PinSource3, GPIO_AF_FSMC );	// A19 -> DC
#endif
...
void STM32f4_Discovery_LCD_Init(void) {
...
#ifdef OPEN407VD
#ifdef SSD1289
	LCD_WriteReg ( SSD1289_DISPLAY_CTRL_REG, 0x0021 );
	LCD_WriteReg ( SSD1289_OSC_START_REG, 0x0001 );
	LCD_WriteReg ( SSD1289_DISPLAY_CTRL_REG, 0x0023 );
	LCD_WriteReg ( SSD1289_SLEEP_MODE_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_DISPLAY_CTRL_REG, 0x0033 );
#if (DISP_ORIENTATION == 0)
	LCD_WriteReg ( SSD1289_ENTRY_MODE_REG, 0x6830 );
#elif (DISP_ORIENTATION == 90)
	LCD_WriteReg ( SSD1289_ENTRY_MODE_REG, 0x6828 );
#elif (DISP_ORIENTATION == 180)
	LCD_WriteReg ( SSD1289_ENTRY_MODE_REG, 0x6810 );
#elif (DISP_ORIENTATION == 270)
	LCD_WriteReg ( SSD1289_ENTRY_MODE_REG, 0x6818 );
#endif
	...
	LCD_WriteReg ( SSD1289_CMP_1_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_CMP_2_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_H_PORCH_REG, 0xEF1C );
	LCD_WriteReg ( SSD1289_V_PORCH_REG, 0x0103 );
	LCD_WriteReg ( SSD1289_FRAME_CYCLE_CTRL_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_GATE_SCAN_START_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_V_SCRL_CTRL_1_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_V_SCRL_CTRL_2_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_FRST_WND_START_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_FRST_WND_END_REG, 0x013F );
	LCD_WriteReg ( SSD1289_SEC_WND_START_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_SEC_WND_END_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_H_RAM_POS_REG, 0xEF00 );
	LCD_WriteReg ( SSD1289_V_RAM_START_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_V_RAM_END_REG, 0x013F );
	...
	LCD_WriteReg ( SSD1289_RAM_WD_MSK_1_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_RAM_WD_MSK_2_REG, 0x0000 );
	LCD_WriteReg ( SSD1289_FRAME_FREQ_REG, 0x8000 );
	LCD_WriteReg ( SSD1289_RAM_DATA_REG, 0x0000 );
#endif

#ifdef ILI9325	// ILI9325 or ILI9328
	LCD_WriteReg ( ILI9325_OSC_START_REG, 0x0001 );	/* start internal osc */

	LCD_WriteReg ( ILI9325_OUTPUT_CTRL_1_REG, (0<<10) | (1<<8) );	/* set SS and SM bit */

	LCD_WriteReg ( ILI9325_LCD_DRIVE_AC_CTRL_REG, 0x0700 ); 	/* power on sequence */
#if (DISP_ORIENTATION == 0)
	LCD_WriteReg ( ILI9325_ENTRY_MODE_REG, (1<<12) | (1<<5) | (1<<4) | (0<<3) );
#elif (DISP_ORIENTATION == 90)
	LCD_WriteReg ( ILI9325_ENTRY_MODE_REG, (1<<12) | (0<<5) | (1<<4) | (1<<3) );
#elif (DISP_ORIENTATION == 180)
	LCD_WriteReg ( ILI9325_ENTRY_MODE_REG, (1<<12) | (0<<5) | (0<<4) | (0<<3) );
#elif (DISP_ORIENTATION == 270)
	LCD_WriteReg ( ILI9325_ENTRY_MODE_REG, (1<<12) | (1<<5) | (0<<4) | (1<<3) );
#endif
	LCD_WriteReg ( ILI9325_RESIZE_CTRL_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_DISPLAY_CTRL_2_REG, 0x0207 );
	LCD_WriteReg ( ILI9325_DISPLAY_CTRL_3_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_DISPLAY_CTRL_4_REG, 0x0000 );	/* display setting */
	LCD_WriteReg ( ILI9325_RGB_DISP_CTRL_1_REG, 0x0001 );	/* display setting */
	LCD_WriteReg ( ILI9325_FRM_MKR_POS_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_RGB_DISP_CTRL_2_REG, 0x0000 );
	/* Power On sequence */
	LCD_WriteReg ( ILI9325_PWR_CTRL_1_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PWR_CTRL_2_REG, 0x0007 );
	LCD_WriteReg ( ILI9325_PWR_CTRL_3_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PWR_CTRL_4_REG, 0x0000 );
	_delay_(50 );  /* delay 50 ms */
	LCD_WriteReg ( ILI9325_PWR_CTRL_1_REG, 0x1590 );
	LCD_WriteReg ( ILI9325_PWR_CTRL_2_REG, 0x0227 );
	_delay_(50 );  /* delay 50 ms */
	LCD_WriteReg ( ILI9325_PWR_CTRL_3_REG, 0x009c );
	_delay_(50 );  /* delay 50 ms */
	LCD_WriteReg ( ILI9325_PWR_CTRL_4_REG, 0x1900 );
	LCD_WriteReg ( ILI9325_PWR_CTRL_7_REG, 0x0023 );
	LCD_WriteReg ( ILI9325_FRM_RATE_CLR_CTRL_REG, 0x000e );
	_delay_(50 );  /* delay 50 ms */
	_delay_(50 );  /* delay 50 ms */
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_1_REG, 0x0007 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_2_REG, 0x0707 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_3_REG, 0x0006 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_4_REG, 0x0704 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_5_REG, 0x1f04 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_6_REG, 0x0004 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_7_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_8_REG, 0x0706 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_9_REG, 0x0701 );
	LCD_WriteReg ( ILI9325_GAMMA_CTRL_10_REG, 0x000f );
	_delay_(50 );  /* delay 50 ms */
	LCD_WriteReg ( ILI9325_H_RAM_START_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_H_RAM_END_REG, 0x00ef );
	LCD_WriteReg ( ILI9325_V_RAM_START_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_V_RAM_END_REG, 0x013f );
	LCD_WriteReg ( ILI9325_OUTPUT_CTRL_2_REG, 0xa700 );
	LCD_WriteReg ( ILI9325_BASE_IMG_DISP_CTRL_REG, 0x0001 );
	LCD_WriteReg ( ILI9325_V_SCRL_CTRL_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_1_DISP_POS_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_1_AREA_S_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_1_AREA_E_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_2_DISP_POS_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_2_AREA_S_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PART_IMG_2_AREA_E_REG, 0x0000 );
	LCD_WriteReg ( ILI9325_PANEL_IFACE_CTRL_1_REG, 0x0010 );
	LCD_WriteReg ( ILI9325_PANEL_IFACE_CTRL_2_REG, 0x0600 );
	LCD_WriteReg ( ILI9325_PANEL_IFACE_CTRL_3_REG, 0x0003 );
	LCD_WriteReg ( ILI9325_PANEL_IFACE_CTRL_4_REG, 0x0110 );
	/* display on sequence */
	LCD_WriteReg ( ILI9325_DISPLAY_CTRL_1_REG, 0x0133 );
	LCD_WriteReg ( ILI9325_RAM_DATA_REG, 0x0000 );
#endif
...
void LCD_SetCursor ( uint16_t Xpos, uint16_t Ypos ) {
	...
	LCD_WriteReg ( SSD1289_X_RAM_ADDR_REG, Xpos );
	/* Set the Y address of the display cursor.*/
	LCD_WriteReg ( SSD1289_Y_RAM_ADDR_REG, Ypos );
#endif
#ifdef ILI9325
#if (DISP_ORIENTATION == 270)
	/* Set the X address of the display cursor.*/
	LCD_WriteReg ( ILI9325_X_RAM_ADDR_REG, LCD_PIXEL_HEIGHT - 1 - Xpos );
	/* Set the Y address of the display cursor.*/
	LCD_WriteReg ( ILI9325_Y_RAM_ADDR_REG, Ypos );
#endif
#endif
#else
	/* Set the X address of the display cursor.*/
	...
	for (index = 0; index < LCD_Currentfonts->Height; index++) {
		...
		for (i = 0; i < LCD_Currentfonts->Width; i++) {
			...
			if ((((c[index] & ((0x80 << ((LCD_Currentfonts->Width / 12) * 8)) >> i)) == 0x00)
					&& (LCD_Currentfonts->Width <= 12)) || (((c[index] & (0x1 << i)) == 0x00)
...
void LCD_DisplayChar ( uint16_t Line, uint16_t Column, uint8_t Ascii ) {
	if ((Ascii >= ' ') && (Ascii <= 'z')) {
		  Ascii -= 32;
	} else if (((Ascii >= 'À') && (Ascii <= 'ß')) || ((Ascii >= 'à') && (Ascii <= 'ÿ'))) {
		  Ascii -= 97;
	} else if ( Ascii == '¨' ) {
		  Ascii -= 9;
	} else if ( Ascii == '¸' ) {
