...
#include "fonts.h"
	...
	LCD_DisplayStringLine ( LINE(5), (uint8_t*) "Статический IP-адрес" );
	...
	LCD_DisplayStringLine ( LINE(7), (uint8_t*) "   Маска подсети    " );
	LCD_DisplayStringLine ( LINE(8), msktxt );

	LCD_SetTextColor ( LCD_COLOR_BLUE );
#ifdef TCP_SERVER
	sprintf ( (char*)porttxt,  "Порт прослушиван.%d  ", LISTEN_PORT );
#else
	sprintf ( (char*) porttxt, "Порт соединения  %d  ", DESTINATION_PORT );
#endif
