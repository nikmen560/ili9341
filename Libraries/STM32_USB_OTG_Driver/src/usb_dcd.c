...
void DCD_Init ( USB_OTG_CORE_HANDLE *pdev, USB_OTG_CORE_ID_TypeDef coreID ) {
	...
	for (i = 0; i < pdev->cfg.dev_endpoints; i++) {
		// Программа сюда заходит "первые" 4 раза (и всего 4 раза),
		//  и, естественно, не осуществляет передачи,
		//  ибо "ep->xfer_buff = 0"
	...
	for (i = 0; i < pdev->cfg.dev_endpoints; i++) {
		// Программа сюда заходит "вторые" 4 раза (и всего 4 раза),
		//  и, естественно, не осуществляет передачи,
		//  ибо "ep->xfer_buff = 0"
...
uint32_t   DCD_EP_PrepareRx ( USB_OTG_CORE_HANDLE *pdev, uint8_t ep_addr, uint8_t *pbuf, uint16_t  buf_len) {
	...
	// qQuantity++;
	// Последовательность идентична "USB_57.txt --- USB_80.txt"
	// qQuantity == 1 - есть достаточно длинный ответ: USB_81.txt, USB_82.txt
	// qQuantity == 2 - есть достаточно длинный ответ: USB_83.txt, USB_84.txt
	// qQuantity == 3 - есть достаточно длинный  ответ: USB_85.txt, USB_86.txt
	// qQuantity == 4 - есть короткий ответ: USB_87.txt, USB_88.txt
	// qQuantity == 5 - есть длинный  ответ: USB_89.txt, USB_90.txt
	// qQuantity == 6 - есть короткий ответ: USB_91.txt, USB_92.txt
	// qQuantity == 7 - есть очень длинный ответ: USB_93.txt, USB_94.txt
	// qQuantity == 8 - есть короткий ответ: USB_95.txt, USB_96.txt
	// qQuantity == 9 - есть достаточно длинный ответ: USB_97.txt, USB_98.txt
	// qQuantity == 10 - есть длинный ответ: USB_99.txt, USB_100.txt
	// qQuantity == 11 - есть короткий ответ: USB_101.txt, USB_102.txt
	// qQuantity == 12 - есть короткий ответ (конец сеанса): USB_103.txt, USB_104.txt
	//	if (qQuantity == 12) {
...
uint32_t DCD_EP_Tx(USB_OTG_CORE_HANDLE *pdev, uint8_t ep_addr, uint8_t *pbuf, uint32_t buf_len) {
	// Программа сюда заходит чаще всего
	//  (23 раза в режиме стандартной инициализации USB-хостом)
