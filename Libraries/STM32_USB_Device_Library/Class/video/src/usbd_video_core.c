...
static uint8_t  usbd_video_EP0_TxSent (void *pdev);
static uint8_t  usbd_video_IsoINIncomplete (void *pdev);
static uint8_t  usbd_video_IsoOUTIncomplete (void *pdev);
...
static uint8_t usbd_video_Setup ( void *pdev, USB_SETUP_REQ *req ) {
	...
	case USB_REQ_TYPE_CLASS:
		...
		case GET_MIN:
		case GET_MAX:
			VIDEO_Req_GetCurrent ( pdev, req );        // повторений запроса, 7 раз
			break;
		case SET_CUR:
