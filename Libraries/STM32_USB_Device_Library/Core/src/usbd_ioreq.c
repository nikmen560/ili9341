...
USBD_Status  USBD_CtlContinueSendData ( USB_OTG_CORE_HANDLE  *pdev, uint8_t *pbuf, uint16_t len ) {
	// qQuantity == 1 - есть короткий ответ: USB_49.txt, USB_50.txt
	// qQuantity == 2 - есть короткий ответ: USB_51.txt, USB_52.txt
	// qQuantity == 3 - есть короткий ответ: USB_53.txt, USB_54.txt
	// qQuantity == 4 - есть короткий ответ (это конец сеанса): USB_55.txt, USB_56.txt
...
USBD_Status  USBD_CtlContinueRx (USB_OTG_CORE_HANDLE  *pdev, uint8_t *pbuf, uint16_t len) {
	// Программа сюда ни разу не заходит
	//  (по крайней мере, в режиме стандартной инициализации USB-хостом)
	USBD_Status ret = USBD_OK;
